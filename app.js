...
function realizado(k){
  const ls=D.lancamentos.filter(l=>ym(l.data)===k);
...
function vLanc(){
  ...
  ${ls.map(l=>`<tr class="${l.protegido||l.beneficio?'dim':''}">
    ...
    <td>${esc(l.descricao)}${l.protegido?' <span class="tag t-g">protegido</span>':''}${l.beneficio?' <span class="tag t-g">benefício</span>':''}</td>
...
function vCad(){
  ...
  return head('Cadastros','Renda, contas fixas e benefícios. Mudar aqui recalcula tudo.')
